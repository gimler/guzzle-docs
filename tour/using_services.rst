...
Similar to RestTemplates in Spring, templates can be specified in the client's ``get``, ``head``, ``delete``, ``post``, ``put`` and ``options`` methods, which allow placeholders to be specified in the the request template that will be overwritten with an array of configuration data referenced by key.
